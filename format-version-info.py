...
def bch_format_string(format_info):
    g = 0b10100110111
    data = format_info << 10
    for i in range(data.bit_length() - 1, 9, -1): # binary polynomial division
        if data & (1 << i):
            data ^= g << (i - 10)
    return (format_info << 10 | data) ^ 0b101010000010010 #xor with fixed masking pattern

#prepare version string
def bch_version_info(version):
    g = 0b1111100100101 
    data = version << 12
    for i in range(data.bit_length() - 1, 11, -1): # binary polynomial division, same as bch_format_string with longer generator polynomial
        if data & (1 << i):
            data ^= g << (i - 12)
...
def place_format_info(matrix, ec_level, mask_pattern):
    ...
    fmt_bits = f'{fmt:015b}' # makes sure format bits are 15 bits long
...
def place_version_info(matrix, version):
    if version < 7: # ensure version >= 7
        return matrix

    bits = f'{bch_version_info(version):018b}' # makes sure version bits are 18 bits long
    for i in range(6):
        for j in range(3):
